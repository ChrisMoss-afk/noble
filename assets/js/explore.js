...
function set_process_stage(active_stage_element, options = {}) {
    ...
    if (!active_stage_element || active_stage_element.classList.contains("is-active")) {
        if (move_focus) {
...
process_stage_elements.forEach((stage_element, stage_index) => {
    ...
    trigger_element.addEventListener("keydown", (event) => {
        ...
        if (["ArrowRight", "ArrowDown"].includes(event.key)) {
            ...
            next_index = (stage_index - 1 + process_trigger_elements.length) % process_trigger_elements.length;
